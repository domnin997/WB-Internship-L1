...
// при разном количестве переменных, создаваемых в функции, максимальное число вызовов отличается
// следовательно, вызов каждой функции занимает определенное место в callstack
// память занимает 1) созданный под функцию контекст исполнения и 2) переменная
...
// где y - это число переменных, constSize - размер одной из них, а z - размер контекста вызова
// аналогично x = 12544*(y*constSize + z); одна буква по системе utf-16 занимает 2 байта памяти, поэтому 
...
function calcStackSize () {
    ...
    try {
        ...
        i++;
        console.log(e)
        console.log('Максимальный размер стека вызовов', i);
    }
}
